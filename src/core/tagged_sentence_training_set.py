...
#
    def build(self):
        sentences = pd.read_sql("select body, (select cast(group_concat(tag_id separator ',') as char) from taggings where taggings.taggable_id = sentences.id) as tag_ids from sentences having tag_ids is not null;", self.db)
